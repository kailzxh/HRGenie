...
//     const res = await fetch(`/api/users/check-exists?email=${encodeURIComponent(email)}`)
//     if (!res.ok) throw new Error('Failed to fetch user info')
//     const data = await res.json()
//     return data // { exists: boolean, role: UserRole }
//   }
...
//       if (!userInfo.exists) {
//         await auth.signOut()
//         throw new Error('No account found. Please contact HR/Admin.')
//       }

//       // 3. Optionally enforce role
//       if (credentials.role && userInfo.role !== credentials.role) {
//         await auth.signOut()
//         throw new Error(`You are registered as ${userInfo.role}, not ${credentials.role}`)
//       }

//       toast.success('Logged in successfully!')

//       // Save user info in context or redirect as needed
//     } catch (err) {
...
//       if (!email) throw new Error('OAuth login failed')
...
//                 {/* Role */}
//                 <div>
//                   <label htmlFor="role" className="block text-sm font-semibold text-white/90 mb-2">Login as</label>
//                   <div className="relative" onBlur={() => setRoleOpen(false)} tabIndex={-1}>
//                     <button id="role" type="button" aria-haspopup="listbox" aria-expanded={roleOpen} onClick={() => setRoleOpen(!roleOpen)} className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-white/60 bg-white/15 backdrop-blur-3xl shadow-inner focus:ring-4 focus:ring-teal-500/30 focus:border-teal-600/60 transition-all text-white font-medium">
//                       <span className="truncate">{credentials.role || 'Select your role'}</span>
//                       <ChevronDown className={`ml-3 h-5 w-5 transition-transform ${roleOpen ? 'rotate-180' : ''}`} />
//                     </button>
//                     {roleOpen && (
//                       <ul role="listbox" className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border border-white/60 bg-white/45 backdrop-blur-5xl shadow-xl ring-1 ring-white/20">
//                         {['', 'employee', 'hr', 'manager', 'admin'].map((value) => {
//                           const selected = (credentials.role || '') === value
//                           const label = value === '' ? 'Select your role' : value === 'hr' ? 'HR Recruiter' : value === 'manager' ? 'Senior Manager' : value === 'admin' ? 'Management Admin' : 'Employee'
//                           return (
//                             <li key={value} role="option" aria-selected={selected} onMouseDown={(e) => e.preventDefault()} onClick={() => { setCredentials({ ...credentials, role: value as UserRole }); setRoleOpen(false) }} className={`flex items-center justify-between px-4 py-2 cursor-pointer text-black hover:bg-white/50 ${selected ? 'bg-white/0' : ''}`}>
//                               <span>{label}</span>
...
//                   <button type="button" onClick={() => handleOAuthLogin('google')} className="w-full inline-flex justify-center py-3 px-4 rounded-xl border border-white/60 bg-white/15 backdrop-blur-3xl text-sm font-medium text-white hover:bg-white/20 transition-colors">
//                     <div className="w-5 h-5 bg-red-500/80 rounded-full mr-2 ring-1 ring-white/40"></div>Google
//                   </button>
//                   <button type="button" onClick={() => handleOAuthLogin('apple')} className="w-full inline-flex justify-center py-3 px-4 rounded-xl border border-white/60 bg-white/15 backdrop-blur-3xl text-sm font-medium text-white hover:bg-white/20 transition-colors">
//                     <div className="w-5 h-5 bg-black/80 rounded-full mr-2 ring-1 ring-white/40"></div>Apple
//                   </button>
//                 </div>
//               </div>
//             </div>
