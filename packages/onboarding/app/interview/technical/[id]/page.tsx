...
import { Application, CodingChallenge, ChatMessage } from '@/lib/supabase/types';
...
export default function TechnicalInterviewPage() {
  ...
  const fetchApplicationAndChallenge = async () => {
    const { data: appData, error: appError } = await supabase
      ...
      .select('*, job:jobs(*)') // remove schema prefix from join
      .eq('id', params.id)
    ...
    if (appData && !appError) {
      ...
      const { data: challengeData } = await supabase
        .from('coding_challenges') // <-- onboarding schema
        .select('*')
        .limit(1)
        .maybeSingle();
      ...
      if (challengeData) {
        ...
        if (!appData.technical_interview_transcript || appData.technical_interview_transcript.length === 0) {
...
Good luck!`,
  ...
  const sendMessage = async () => {
    ...
    const assistantMessage: ChatMessage = {
      role: 'assistant',
      content: "I'm processing your message. In a production environment, this would connect to the Vertex AI Agent Builder to provide intelligent responses based on your question and the current interview context.",
      timestamp: new Date().toISOString(),
    };
